...
*   **交战区域（向炮声前进）：** 以防守方为中心，切比雪夫距离 ≤2（5x5网格范围）内，且 AP 满足最低消耗阈值（4 AP）的存活单位或在攻击范围内的炮兵类单位，皆可被编入战斗池。
...
#### 2.5.1 阶跃式疲劳度系统
疲劳度（Fatigue）采用“**规则阈值 + 实现安全上限**”双口径：
*   **规则阈值（设计口径）：** 0 到 8。
*   **实现安全上限（代码口径）：** 0 到 20（仅作数值缓冲，不改变规则判定阈值）。
*   **常态区 (0 - 4)：** 100% 最大 AP，无惩罚。
*   **疲惫区 (5 - 6)：** 最大 AP 扣除 20%，攻防力量 x0.9。
*   **力竭区 (7 - 8)：** 最大 AP 扣除 50%，攻防力量 x0.5。
*   **超过 8：** 组织涣散，ZOC失效，AP归0，极易被消灭（9 到 20 仍按该状态处理）。
...
#### 2.5.4 组织涣散回补重整机制（已实现）
当部队处于组织涣散状态（$Fatigue > 8$）且在回合结算时位于补给范围内，下回合疲劳度强制设为 8：

$$
Fatigue_{next} = 8 \quad \text{if } Fatigue_{current} > 8 \land InSupply = true
$$

该规则优先于常规的 AP 阶梯疲劳恢复（$AP \ge 8$ 减2、$4 \le AP < 8$ 减1）。
...
### 2.8 战线表现与渲染机制
1.  **边界提取：** 基于现有控制图遍历异色交界点，并按连通段输出前线链。
